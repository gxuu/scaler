...
class SchedulerProcess(multiprocessing.get_context("spawn").Process):  # type: ignore[misc]
    ...
    def __register_signal(loop):
        loop.add_signal_handler(signal.SIGINT, functools.partial(SchedulerProcess.__handle_signal))
        loop.add_signal_handler(signal.SIGTERM, functools.partial(SchedulerProcess.__handle_signal))
