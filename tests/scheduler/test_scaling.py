...
class TestCapabilityScalingController(unittest.TestCase):
    ...
    def test_starts_worker_group_when_no_capable_workers(self):
        ...
        commands = self.controller.get_scaling_commands(
            snapshot, adapter_heartbeat, self.worker_groups, self.worker_group_capabilities
        )
    ...
    def test_on_scaling_command_response_start_success(self):
        """Test that on_scaling_command_response correctly updates state on successful start."""
        ...
        updated_groups, updated_caps = self.controller.on_scaling_command_response(
            response, self.worker_groups, self.worker_group_capabilities
        )

        self.assertIn(b"wg-new", updated_groups)
        self.assertIn(b"wg-new", updated_caps)
        self.assertEqual(updated_caps[b"wg-new"], {"gpu": -1})

    def test_on_scaling_command_response_shutdown_success(self):
        """Test that on_scaling_command_response correctly updates state on successful shutdown."""
        # Pre-populate state
        worker_groups: WorkerGroupState = {b"wg-old": [WorkerID(b"worker-1")]}
        worker_group_caps: WorkerGroupCapabilities = {b"wg-old": {"gpu": -1}}
        ...
        updated_groups, updated_caps = self.controller.on_scaling_command_response(
            response, worker_groups, worker_group_caps
        )

        self.assertNotIn(b"wg-old", updated_groups)
        self.assertNotIn(b"wg-old", updated_caps)
    ...
    def test_no_duplicate_worker_group_for_pending_group(self):
        ...
        commands1 = self.controller.get_scaling_commands(
            snapshot1, adapter_heartbeat, self.worker_groups, self.worker_group_capabilities
        )
        ...
        updated_groups, updated_caps = self.controller.on_scaling_command_response(
            wg_response, self.worker_groups, self.worker_group_capabilities
        )
        ...
        commands2 = self.controller.get_scaling_commands(snapshot2, adapter_heartbeat, updated_groups, updated_caps)
