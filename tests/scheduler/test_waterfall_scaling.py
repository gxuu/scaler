...
from scaler.scheduler.controllers.policies.utility import create_policy_controller
...
def _create_adapter_heartbeat(adapter_id: bytes, max_worker_groups: int = 10) -> WorkerAdapterHeartbeat:
    return WorkerAdapterHeartbeat.new_msg(
        max_worker_groups=max_worker_groups, workers_per_group=1, capabilities={}, worker_manager_adapter_id=adapter_id
    )
...
class TestWaterfallScalingPolicy(unittest.TestCase):
    ...
    def setUp(self):
        setup_logger()
        self.rules = [
            WaterfallRule(priority=1, worker_manager_adapter_id=b"adapter_a", max_workers=10),
            WaterfallRule(priority=2, worker_manager_adapter_id=b"adapter_b", max_workers=20),
        ]
    ...
    def test_single_priority_scale_up(self):
        ...
        rules = [WaterfallRule(priority=1, worker_manager_adapter_id=b"adapter_a", max_workers=10)]
        ...
        commands = policy.get_scaling_commands(snapshot, heartbeat, worker_groups, worker_group_caps, adapter_snapshots)
    ...
    def test_adapter_not_in_config(self):
        """Adapter with unknown worker_manager_adapter_id should receive no commands."""
        tasks = _create_tasks(5)
        snapshot = InformationSnapshot(tasks=tasks, workers={})
        worker_groups: WorkerGroupState = {}
        worker_group_caps: WorkerGroupCapabilities = {}
    ...
    def test_effective_capacity_min_config_and_heartbeat(self):
        ...
        rules = [WaterfallRule(priority=1, worker_manager_adapter_id=b"adapter_a", max_workers=5)]
        policy = WaterfallScalingPolicy(rules)
    ...
    def test_same_priority_concurrent_scaling(self):
        ...
        rules = [
            WaterfallRule(priority=1, worker_manager_adapter_id=b"adapter_a", max_workers=10),
            WaterfallRule(priority=1, worker_manager_adapter_id=b"adapter_b", max_workers=10),
        ]
        policy = WaterfallScalingPolicy(rules)
    ...
    def test_prefix_matching_with_runtime_ids(self):
        ...
        rules = [
            WaterfallRule(priority=1, worker_manager_adapter_id=b"NAT", max_workers=10),
            WaterfallRule(priority=2, worker_manager_adapter_id=b"ECS", max_workers=20),
        ]
        policy = WaterfallScalingPolicy(rules)
