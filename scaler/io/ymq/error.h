...
struct Error: std::exception {
    ...
    Error(ErrorCode e, Args&&... args) noexcept: _errorCode(e) {
        Timestamp z;
        _logMsg = argsToString(z, convertErrorToExplanation(e), std::forward<Args>(args)...);
    }

    static const char* convertErrorToExplanation(ErrorCode e) { return _errorToExplanation[e]; }

    const char* what() const noexcept override { return _logMsg.c_str(); }

    constexpr const static std::array<const char*, ErrorCode::End + 1> _errorToExplanation {
        "",
        "Invalid port format, example input \"tcp://127.0.0.1:2345\"",
